...
if __name__ == "__main__":
    ...
    @app.route('/api/cartillas/by_email', methods=['POST'])
def get_cartillas_by_email():
    ...
    with db_session() as db:
        # Get all vouchers for this email
        vouchers = db.query(Voucher).filter(
            Voucher.email.ilike(email)
        ).all()
        ...
        # Get cartillas linked to those vouchers
        cartillas = db.query(Cartilla).filter(
            Cartilla.voucher_code.in_(voucher_codes)
        ).all()

        # Filter by active session
        if active_sid:
            cartillas = [c for c in cartillas if c.session_id == active_sid]

        return jsonify({
            "cartillas": [c.to_dict() for c in cartillas]
        })